...
class TmdbScraper:
    ...
    def gen_scraper_files(
        ...
        file_path: Path,
        transfer_type: str,
        metainfo: MetaBase = None,
        force_nfo: bool = False,
        force_img: bool = False,
    ):
        """
        ...
        try:
            ...
            if mediainfo.type == MediaType.MOVIE:
                # 不已存在时才处理
                if self._force_nfo or (
                    not file_path.with_name("movie.nfo").exists()
                    and not file_path.with_suffix(".nfo").exists()
                ):
                    #  生成电影描述文件
                    self.__gen_movie_nfo_file(mediainfo=mediainfo, file_path=file_path)
                # 生成电影图片
                for attr_name, attr_value in vars(mediainfo).items():
                    if (
                        ...
                        if self._force_img or not image_path.exists():
                            self.__save_image(url=attr_value, file_path=image_path)
            # 电视剧，路径为每一季的文件名 名称/Season xx/名称 SxxExx.xxx
                ...
                if meta.begin_season is None:
                    meta.begin_season = (
                        mediainfo.season if mediainfo.season is not None else 1
                    )
                # 根目录不存在时才处理
                if (
                    self._force_nfo
                    or not file_path.parent.with_name("tvshow.nfo").exists()
                ):
                    # 根目录描述文件
                    self.__gen_tv_nfo_file(
                        mediainfo=mediainfo, dir_path=file_path.parents[1]
                    )
                # 生成根目录图片
                for attr_name, attr_value in vars(mediainfo).items():
                    if (
                        ...
                        if self._force_img or not image_path.exists():
                            self.__save_image(url=attr_value, file_path=image_path)
                # 查询季信息
                cn_seasoninfo = self.tmdb.get_tv_season_detail(
                    mediainfo.tmdb_id, meta.begin_season
                )
                seasoninfo = Get_TW_info.get_tv_season_detail(
                    mediainfo.tmdb_id, meta.begin_season, cn_info=cn_seasoninfo
                )
                if seasoninfo:
                    # 季目录NFO
                    if (
                        self._force_nfo
                        or not file_path.with_name("season.nfo").exists()
                    ):
                        self.__gen_tv_season_nfo_file(
                    ...
                    if seasoninfo.get("poster_path"):
                        ...
                        image_path = file_path.parent.with_name(
                            f"season{sea_seq}-poster{ext}"
                        )
                        if self._force_img or not image_path.exists():
                ...
                if episodeinfo:
                    ...
                    if self._force_nfo or not file_path.with_suffix(".nfo").exists():
                        self.__gen_tv_episode_nfo_file(
                    ...
                    if episode_image:
                        image_path = file_path.with_name(
                            file_path.stem + "-thumb.jpg"
                        ).with_suffix(Path(episode_image).suffix)
                        if self._force_img or not image_path.exists():
                            self.__save_image(
    ...
    def __gen_common_nfo(mediainfo: MediaInfo, doc, root):
        ...
        DomUtils.add_node(doc, root, "outline", mediainfo.overview or "")
    ...
    def __gen_movie_nfo_file(self, mediainfo: MediaInfo, file_path: Path):
        """
        生成电影的NFO描述文件
    ...
    def __gen_tv_nfo_file(self, mediainfo: MediaInfo, dir_path: Path):
        """
        生成电视剧的NFO描述文件
        :param mediainfo: 媒体信息
    ...
    def __gen_tv_season_nfo_file(
        self, seasoninfo: dict, season: int, season_path: Path
    ):
        """
        生成电视剧季的NFO描述文件
        ...
        # 标题
        DomUtils.add_node(doc, root, "title", "第 %s 季" % season)
        # 发行日期
    ...
    def __gen_tv_episode_nfo_file(
        self, tmdbid: int, episodeinfo: dict, season: int, episode: int, file_path: Path
    ):
        """
        生成电视剧集的NFO描述文件
        :param tmdbid: TMDBID
        ...
        # 简介
        DomUtils.add_node(doc, root, "plot", episodeinfo.get("overview") or "")
        DomUtils.add_node(doc, root, "outline", episodeinfo.get("overview") or "")
        # 发布日期
    ...
    def __save_image(self, url: str, file_path: Path):
        ...
        try:
            ...
            r = RequestUtils(proxies=settings.PROXY).get_res(
